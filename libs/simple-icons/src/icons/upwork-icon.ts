...
@Component({
  selector: 'svg[siUpworkIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Upwork</svg:title>
    <svg:path
...
export class SiUpworkIcon {
  ...
  readonly ariaLabel = input<string>('Upwork icon');
  readonly ariaHidden = input<boolean | 'true' | 'false'>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#6FDA44');
}
