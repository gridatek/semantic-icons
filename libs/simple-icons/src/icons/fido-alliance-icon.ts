...
@Component({
  selector: 'svg[siFidoAllianceIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Fido Alliance</svg:title>
    <svg:path
  ...
  host: {
    ...
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#FFBF3B',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiFidoAllianceIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  // Accessibility inputs
  readonly ariaLabel = input<string>('Fido Alliance icon');
  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#FFBF3B');
}
