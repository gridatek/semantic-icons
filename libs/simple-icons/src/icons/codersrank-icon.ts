...
@Component({
  selector: 'svg[siCodersrankIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>CodersRank</svg:title>
    <svg:path
  ...
  host: {
    ...
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#67A4AC',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiCodersrankIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  // Accessibility inputs
  readonly ariaLabel = input<string>('CodersRank icon');
  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#67A4AC');
}
