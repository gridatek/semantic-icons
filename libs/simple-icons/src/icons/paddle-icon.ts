...
@Component({
  selector: 'svg[siPaddleIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Paddle</svg:title>
    <svg:path
...
export class SiPaddleIcon {
  ...
  readonly ariaLabel = input<string>('Paddle icon');
  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#FDDD35');
}
