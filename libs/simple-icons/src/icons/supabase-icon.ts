...
@Component({
  selector: 'svg[siSupabaseIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Supabase</svg:title>
    <svg:path
...
export class SiSupabaseIcon {
  ...
  readonly ariaLabel = input<string>('Supabase icon');
  readonly ariaHidden = input<boolean | 'true' | 'false'>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#3FCF8E');
}
