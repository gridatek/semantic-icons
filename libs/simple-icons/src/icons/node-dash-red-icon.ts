...
@Component({
  selector: 'svg[si-node-dash-red-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Node-RED</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Node-RED icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#8F0000',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiNodeDashREDIcon {
  ...
  readonly fill = input<string>('#8F0000');
}
