...
@Component({
  selector: 'svg[siVirtualboxIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>VirtualBox</svg:title>
    <svg:path
  ...
  host: {
    ...
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#2F61B4',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiVirtualboxIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  // Accessibility inputs
  readonly ariaLabel = input<string>('VirtualBox icon');
  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#2F61B4');
}
