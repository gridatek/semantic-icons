...
@Component({
  selector: 'svg[siVolkswagenIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Volkswagen</svg:title>
    <svg:path
...
export class SiVolkswagenIcon {
  ...
  readonly ariaLabel = input<string>('Volkswagen icon');
  readonly ariaHidden = input<boolean | 'true' | 'false'>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#151F5D');
}
