...
@Component({
  selector: 'svg[siDevDotToIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>dev.to</svg:title>
    <svg:path
  ...
  host: {
    ...
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#0A0A0A',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiDevDotToIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  // Accessibility inputs
  readonly ariaLabel = input<string>('dev.to icon');
  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#0A0A0A');
}
