...
@Component({
  selector: 'svg[siScrutinizerCiIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Scrutinizer CI</svg:title>
    <svg:path
  ...
  host: {
    ...
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#8A9296',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiScrutinizerCiIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  // Accessibility inputs
  readonly ariaLabel = input<string>('Scrutinizer CI icon');
  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#8A9296');
}
