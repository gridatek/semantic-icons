...
@Component({
  selector: 'svg[si-owasp-dependency-dash-check-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>OWASP Dependency-Check</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "OWASP Dependency-Check icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#F78D0A',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiOWASPDependencyDashCheckIcon {
  ...
  readonly fill = input<string>('#F78D0A');
}
