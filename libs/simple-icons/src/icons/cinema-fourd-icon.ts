...
@Component({
  selector: 'svg[si-cinema-fourd-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Cinema 4D</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Cinema 4D icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#011A6A',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiCinemaFourdIcon {
  ...
  readonly fill = input<string>('#011A6A');
}
