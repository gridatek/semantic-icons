...
@Component({
  selector: 'svg[siPhabricatorIcon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Phabricator</svg:title>
    <svg:path
...
export class SiPhabricatorIcon {
  ...
  readonly ariaLabel = input<string>('Phabricator icon');
  readonly ariaHidden = input<boolean | 'true' | 'false'>(true);

  readonly viewBox = input<string>('0 0 24 24');

  readonly fill = input<string>('#4A5F88');
}
