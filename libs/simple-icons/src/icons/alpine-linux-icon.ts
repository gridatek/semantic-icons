...
@Component({
  selector: 'svg[si-alpine-linux-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Alpine Linux</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Alpine Linux icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#0D597F',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiAlpineLinuxIcon {
  ...
  readonly fill = input<string>('#0D597F');
}
