...
import { LucideLabIconsGeneratorSchema } from './schema';

export async function lucideLabIconsGenerator(
  tree: Tree,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  options: LucideLabIconsGeneratorSchema,
) {
  const iconsLibPath = 'libs/lucide-lab-icons';

  const iconsSourcePath = 'node_modules/@lucide/lab/icons';
...
function generateIconsComponents(
  ...
  updateJson(tree, path.join(iconsLibPath, 'package.json'), (packageJson) => {
    packageJson.description = `Icons generated based on lucide-lab-icons v${packageVersion}`;
    return packageJson;
  });
}

export default lucideLabIconsGenerator;
