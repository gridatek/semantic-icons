import {
  ChangeDetectionStrategy,
  Component,
  ViewEncapsulation,
  input,
} from '@angular/core';

@Component({
  selector: 'svg[siThermometerIcon]',
  standalone: true,
  imports: [],
  template: `
    <!--! Font Awesome Free 7.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) Copyright 2026 Fonticons, Inc. -->
    <svg:path
      fill="currentColor"
      d="M160 446.1L160 357.3C160 342.4 165.9 328.2 176.4 317.7L198.1 296L239.1 337C248.5 346.4 263.7 346.4 273 337C282.3 327.6 282.4 312.4 273 303.1L232 262.1L278.1 216L319.1 257C328.5 266.4 343.7 266.4 353 257C362.3 247.6 362.4 232.4 353 223.1L312 182.1L358.1 136L399.1 177C408.5 186.4 423.7 186.4 433 177C442.3 167.6 442.4 152.4 433 143.1L392 102.1L399.8 94.3C419.2 74.9 445.4 64.1 472.8 64.1C529.8 64.1 576 110.3 576 167.3C576 194.7 565.1 220.9 545.8 240.3L322.3 463.6C311.8 474.1 297.6 480 282.7 480L193.9 480L104.9 569C95.5 578.4 80.3 578.4 71 569C61.7 559.6 61.7 544.4 71 535L160 446z"
    />
  `,
  host: {
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiThermometerIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  readonly ariaHidden = input<boolean | 'true' | 'false'>(true);

  readonly viewBox = input<string>('0 0 640 640');

  readonly fill = input<string>('currentColor');
}
