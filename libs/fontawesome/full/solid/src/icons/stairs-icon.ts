import {
  ChangeDetectionStrategy,
  Component,
  ViewEncapsulation,
  input,
} from '@angular/core';

@Component({
  selector: 'svg[siStairsIcon]',
  standalone: true,
  imports: [],
  template: `
    <!--! Font Awesome Free 7.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) Copyright 2026 Fonticons, Inc. -->
    <svg:path
      fill="currentColor"
      d="M416 128C416 110.3 430.3 96 448 96L576 96C593.7 96 608 110.3 608 128C608 145.7 593.7 160 576 160L480 160L480 256C480 273.7 465.7 288 448 288L352 288L352 384C352 401.7 337.7 416 320 416L224 416L224 512C224 529.7 209.7 544 192 544L64 544C46.3 544 32 529.7 32 512C32 494.3 46.3 480 64 480L160 480L160 384C160 366.3 174.3 352 192 352L288 352L288 256C288 238.3 302.3 224 320 224L416 224L416 128z"
    />
  `,
  host: {
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiStairsIcon {
  protected readonly xmlns = 'http://www.w3.org/2000/svg';

  readonly ariaHidden = input<boolean | string>(true);

  readonly viewBox = input<string>('0 0 640 640');

  readonly fill = input<string>('currentColor');
}
