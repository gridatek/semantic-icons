...
@Component({
  selector: 'svg[si-one-password-icon]',
  standalone: true,
  imports: [],
  template: `
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "1Password icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#3B66BC',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiOnePasswordIcon {
  ...
  readonly fill = input<string>('#3B66BC');
}
