...
@Component({
  selector: 'svg[si-traefik-proxy-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Traefik Proxy</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Traefik Proxy icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#24A1C1',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiTraefikProxyIcon {
  ...
  readonly fill = input<string>('#24A1C1');
}
