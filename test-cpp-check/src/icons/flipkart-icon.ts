...
@Component({
  selector: 'svg[si-flipkart-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Flipkart</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Flipkart icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#2874F0',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiFlipkartIcon {
  ...
  readonly fill = input<string>('#2874F0');
}
