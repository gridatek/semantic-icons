...
@Component({
  selector: 'svg[si-tl-dash-qu-bec-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Télé-Québec</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Télé-Québec icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#1343FB',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiTLDashQuBecIcon {
  ...
  readonly fill = input<string>('#1343FB');
}
