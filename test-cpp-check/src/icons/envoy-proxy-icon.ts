...
@Component({
  selector: 'svg[si-envoy-proxy-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Envoy Proxy</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Envoy Proxy icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#AC6199',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiEnvoyProxyIcon {
  ...
  readonly fill = input<string>('#AC6199');
}
