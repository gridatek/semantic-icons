...
@Component({
  selector: 'svg[si-apollo-graphql-icon]',
  standalone: true,
  imports: [],
  template: `
    <svg:title>Apollo GraphQL</svg:title>
    <svg:path
  ...
  host: {
    role: 'img',
    '[attr.aria-label]': 'ariaLabel() || "Apollo GraphQL icon"',
    '[attr.aria-hidden]': 'ariaHidden()',
    '[attr.xmlns]': 'xmlns',
    '[attr.viewBox]': 'viewBox()',
    '[attr.fill]': 'fill()',
    'data-slot': 'icon',
    'data-brand-color': '#311C87',
  },
  styles: ``,
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SiApolloGraphqlIcon {
  ...
  readonly fill = input<string>('#311C87');
}
